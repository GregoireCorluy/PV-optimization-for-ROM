...
def Compute_density(data_path, x_element, y_element, chosen_layer, nbr_neighbours, file_type = "mat", filenameT = None, filenameF = None):
    ...
    Returns:
        _type_: Returns the data density in every point of the manifold.
    """
    
    if(file_type.lower() == "mat"):
        array_x = load_array(data_path, x_element, chosen_layer, True)
        array_y = load_array(data_path, y_element, chosen_layer, True)
...
def visualize_species_layer(data_path, species_name, idx_layer):
    ...
    plt.imshow(matrix_species, cmap='viridis') #, interpolation='none'
