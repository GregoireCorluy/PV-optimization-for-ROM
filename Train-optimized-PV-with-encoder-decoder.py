"""
PV optimization on hydrogen datasets using an encoder-decoder architecture (Kamila Zdybał)
Version: Train multiple times the same NN with different seeds
...
other_species_removed = ["logH2O2-10", "logH2O2-20"]
...
list_species_output = ['logH2O2-10', 'H2O', 'H2', 'HO2', 'N2O', 'NO2', 'NO', 'O2', 'OH']
...
if(dataset_type == "autoignition_augm"):
...
for my_seed in range(nbr_seeds):
    ...
    optimizer = get_optimizer(model.parameters(), optimizer_name, lr, optimizer_alpha, optimizer_momentum)
    loss_criterion = get_loss_criterion(loss_name)
    ...
    variable_data = {"Training_id": training_id, "training_name":dirs.training_name,"model_name": dirs.dirout, "curve_name": dirs.dircurves, "metadata_name": dirs.dirMetadata,"data_layer": layer, "nbr datapoints": datapoints, "max epo": max_epo, "epo best": epo_best_model, "optimizer": optimizer_name,
                        ...
                        "list idx species removed Kreg": list_idx_species_removed_Kreg, "always rescale PV": always_rescale_PV}
